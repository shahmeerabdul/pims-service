...
const AdminTFirstMonthResultsPage: React.FC = () => {
        ...
        try {
          ...
          if (status === 'SUCCESS' && file_url) {
            ...
            const filename = exportType === 'daily_entries'
              ? 't_first_month_daily_entries_export.csv'
              : `t_first_month_results_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleExportDailyEntries = async () => {
    try {
      ...
      const response = await questionnairesApi.triggerAdminDailyEntriesExport(selectedGroup, 'PRE_T_1M');
      setExportingId(response.data.task_id);
      setError(null);
    } catch (err) {
      console.error('Failed to export T-First-Month daily entries data', err);
      setError('Failed to initiate CSV export. Please check server logs.');
      setExportStatus(null);
      setExportType(null);
    }
