...
const AdminT2ResultsPage: React.FC = () => {
        ...
        try {
          ...
          if (status === 'SUCCESS' && file_url) {
            ...
            const filename = exportType === 'daily_entries'
              ? 't2_daily_entries_export.csv'
              : `t2_results_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleExportDailyEntries = async () => {
    try {
      ...
      const response = await questionnairesApi.triggerAdminDailyEntriesExport(selectedGroup, 'PRE_T2');
      setExportingId(response.data.task_id);
      setError(null);
    } catch (err) {
      console.error('Failed to export T2 daily entries data', err);
      setError('Failed to initiate CSV export. Please check server logs.');
      setExportStatus(null);
      setExportType(null);
    }
